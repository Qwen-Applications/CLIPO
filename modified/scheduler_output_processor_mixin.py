        ...
        if self.is_generation:
            ...
            for i, (req, next_token_id) in enumerate(zip(batch.reqs, next_token_ids)):
                ...
                if req.is_chunked <= 0:
                    ...
                    if ( False and # MODIFIED: return the last layer hidden states
    ...
    def process_batch_result_decode(
        ...
        for i, (req, next_token_id) in enumerate(zip(batch.reqs, next_token_ids)):
            ...
            if req.return_hidden_states and logits_output.hidden_states is not None:
                # MODIFIED: return the last layer hidden states
                if req.hidden_states == []:
                    req.hidden_states = logits_output.hidden_states[i].detach().cpu().clone()
                else:
                    req.hidden_states += logits_output.hidden_states[i].detach().cpu()
                if req.finished():
                    req.hidden_states = [req.hidden_states / len(req.output_ids)]
                    # req.hidden_states.append(
                    #     logits_output.hidden_states[i].detach().cpu().clone()
                    # )
                # print(
                #     f"[decode step] batch_idx={i}, rid={req.rid}, "
                #     f"token_id={next_token_id}, finished={req.finished()}, "
                #     f"len_output_ids={len(req.output_ids)}, len_hidden_states={len(req.hidden_states)}, "
                #     f"shape_logits_output_hidden_states={logits_output.hidden_states.shape}"
                # )
