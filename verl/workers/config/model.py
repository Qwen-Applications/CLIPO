...
class HFModelConfig(BaseConfig):
    ...
    enable_con_lm_head: bool = False
    # linear or mlp, hidden_size -> [hidden_size] -> con_lm_head_output_size
    con_lm_head_type: str = "linear"
    # Output size of con_lm_head, default to 512
    con_lm_head_output_size: int = 512

    # Loss function for con_lm_head: soft_nn_loss, supcon_loss_in, supcon_loss_out, infonce_loss
    con_lm_head_loss_type: str = "infonce_loss"
    con_lm_head_distance_measure: str = "cosine_distance"
    # -1 do not mask, 0 mask incorrect labels, 1 mask correct labels
    con_lm_head_masked_label: int = 0
    # sim temperature, logit = sim / temperature
    con_lm_head_temperature: float = 0.05

    # optimizer params
    con_lm_head_lr: float = 0.001
    con_lm_head_weight_decay: float = 0.01

    # contrastive_reward = contant_reward - lambda * loss
    con_lm_head_contant_reward: float = 0
    con_lm_head_lambda: float = 0.2

    # path to load and save contrastive head
    con_lm_head_load_path: Optional[str] = None
    con_lm_head_save_path: Optional[str] = None

    # whether to fix contrastive head
    con_lm_head_fixed_head: bool = False

    # whether to skip the trivial samples (all positive or all negative)
    con_lm_head_skip_trivial: bool = True

    # contrastive level: batch_level, group_level
    con_lm_head_contrastive_level: str = "group_level"

    # debias method: layer_norm, mean_subtract
    con_lm_head_debias: Optional[str] = None
