...
def _post_process_outputs(processing_class, output, return_hs=False):
    try:
    ...
    def _map_each_response(resp):
        ...
        hidden_states = resp["meta_info"]["hidden_states"][-1] if return_hs else None
        log_probs, output_token_ids = zip(
            *[(log_prob, token_ids) for log_prob, token_ids, _ in output_token_logprobs], strict=True
        )
        return torch.tensor(output_token_ids), torch.tensor(log_probs), hidden_states

    out_map = map(lambda x: _map_each_response(x), output)
    batched_output_token_ids = []
    batched_logprobs = []
    batched_hidden_states = []
    for output_token_ids, log_probs, hidden_states in out_map:
    ...
    batched_hidden_states = torch.stack(batched_hidden_states, dim=0) if return_hs else None
    return batched_output_token_ids, batched_logprobs, batched_hidden_states
...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        # Get enable_return_hidden_states from config, default to False
        self.enable_return_hidden_states = self.config.get("enable_return_hidden_states", False)
        # initialize the inference engine
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self._tp_rank == 0:
            # print(f"Generate start time: {time.time()}")
            ...
            output = loop.run_until_complete(
                self._engine.async_generate(
                    ...
                    return_hidden_states=self.enable_return_hidden_states,
                )
            )
            # print(f"Generate end time: {time.time()}")
            print(f"Generate {len(output)} rollouts, used {int(time.time() - start_time)} seconds")
        else:
            output = None
        ...
        out = _post_process_outputs(self.processing_class, output, return_hs=self.enable_return_hidden_states)
        ...
        if self.enable_return_hidden_states:
            hidden_states = out[2].to(idx.device)
